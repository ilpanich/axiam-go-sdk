...
- **F-28-01 — `CONTRACT.md` (1.49), `openapi.json` and
  ...
  `go run ./internal/cmd/genmanagement`, nothing hand-edited: **160 → 162
  operations across the same 24 namespaces**. `OAuth2Clients()` gains
  `CreateRegistrationToken` (`POST /api/v1/oauth2-clients/registration-tokens`)
  and `ListRegistrationTokens` (`GET` on the same path) — the T21.4
  ...
  the one-time `InitialAccessToken`), `RegistrationTokenResponse`, and
  `CimdPolicy` (T21.5 client ID metadata documents), which appears as an
  optional `Cimd` field on `OidcPolicy`, `SetOrgSettings` and
  `TenantSettingsOverride`. The `Dcr*` field docs now say "externally
...
- **CONTRACT.md §10.4 — an optional session-revocation feed poller (contract
  1.44).** `axiam.NewRevocationFeed`, attached with
  `JWKSVerifier.WithRevocationFeed`, plus `axiam.RevocationEntryFor` and the
  three bounds the contract names.
  ...
  A `*RevocationFeed` is safe for concurrent use, so one instance can serve
  several guards.

- `Claims.SessionID`, parsed from the OIDC Core `sid` claim; empty when the
  token carries none, which keeps "no session behind this token" distinct from
  "a session named the empty string".

- `jwks.ErrSessionRevoked`, reported when an attached feed lists a token's
  session. Deliberately its own sentinel: "the session behind this token is
  gone" is not the same report as "this credential was never valid", and a
  guard that conflated them would tell a logged-out user their token expired.
...
- Two tests pinning CONTRACT.md §16 against the server's new answer for a
  ...
  changed: §16.3 already retried `5xx` on an eligible operation and this SDK
  already parsed `Retry-After` into `NetworkError.RetryAfter` and honoured it as
  a floor. Both halves are asserted through the public surface with a wire
...
- **A malformed `mtls_endpoint_aliases` entry now returns an error instead of
  ...
  `WithClientCertificate` never reads the member at all.
...
- **RFC 8705 §5 `mtls_endpoint_aliases` (SDK contract 1.40, CONTRACT.md §21.3
  ...
  (the new `MtlsEndpointAliases` type), and the §12 helpers now prefer an alias
  over the top-level entry of the same name on any call made over mutual TLS —
  that is, from a client built with `WithClientCertificate`. Six operations
  ...
  A nil field means "this deployment terminates mutual TLS on the issuer's own
  host", never "mTLS is unsupported": a client without it keeps using the
  conventional endpoints instead of failing. Every field of
  `MtlsEndpointAliases` is `omitempty`, so an endpoint a partial object does
...
- Regenerated the §27 management surface from the vendored
  ...
  `go run ./internal/cmd/genmanagement --check` is green again.
...
- **Contract 1.38: the four public "Sign in with X" operations.** `SsoProviders`,
  ...
  CONTRACT.md §12.2 Go names, on the same `*axiam.Client` as the nine §12
  operations that came before. New exported types `FederationProvider` and
  `FederationProviderList`, matching the §12.1 SDK-type table, plus the
  `ProtocolOidcConnect`/`ProtocolOAuth2`/`ProtocolSaml` discriminants and the
  `HandoffQueryParam`/`HandoffCodeTTL` constants. Upstream: ilpanich/axiam#398.
  ...
  - **`Protocol` selects the start operation** (§12.1 note 10), never
    `ProviderKind`. It is the wire string rather than a Go enum, so a value added
    server-side cannot become a decode failure for the whole list.
...
- Regenerated the §27 management surface (`go run ./internal/cmd/genmanagement`),
  ...
  `management_models.go` and the generated surface test move with it. The
...
- **§5.2.2 rule 4 is new, and is an errata rather than a wire change.** The
  ...
  `X-Tenant-ID` is set in one place, `decorateRequest` in `client.go`,
  unconditionally for every request; no endpoint is special-cased.
