...
// absent until T21.2: adding it before the rest of the server understood
// one would have let an operator register a client whose authentication is
// silently skipped. The server understands one now (`token.rs`'s
// `authenticate_client_credential` has an arm that accepts *no* credential
// and refuses a presented one, the authorization endpoint derives its PKCE
// requirement from this enum, and the admin API refuses the method
// alongside any grant or binding that contradicts it), so the variant
// exists — and only that arm may ever treat a missing credential as
// success.
...
type CreateOAuth2ClientRequest struct {
	// AllowedResources T21.3 / RFC 8707 — the target services this client may name in a
	// `resource` parameter, at `/oauth2/authorize`, `/oauth2/par`,
	// `/oauth2/device_authorization` and `/oauth2/token`. Each entry must be
	// an absolute URI without a fragment (RFC 8707 §2). Entries are stored
	// in their RFC 3986 §6.2.2 normalised form, which is what the read-back
	// shows and what every comparison uses; matching is by equivalence and
	// **never by prefix**. Empty (the default) means the client may name no
	// resource, so every token it obtains carries `axiam:user` or `axiam:m2m`
	// exactly as before RFC 8707 support existed. This is also the list the
	// RFC 8693 token exchange consults for its `audience`/`resource` target.
...
// ManagedBy Who created a client registration (D5, T21.4). The discriminator that
// separates a registration an administrator made from one that arrived
// over an open endpoint. Three things read it and each would otherwise
// have to infer provenance from something that is not provenance: *
...
// every other value (D4) — an unrelated party gets a question put to the
...
// however long it sits unused. [`Admin`](Self::Admin) is the serde default
// and therefore what every row written before T21.4 decodes to, which is
// the truth: they were all created through `POST /oauth2-clients` by
// somebody holding `oauth2_clients:create`.
type ManagedBy string

// The ManagedBy values the server defines. The type is a plain string, so a value
// this SDK's copy of the spec does not list still decodes rather than
// failing the response it arrived in (CONTRACT §27.11 rule 1) — a switch
// over these constants needs a default arm.
const (
	ManagedByAdmin ManagedBy = "admin"
	ManagedByDcr   ManagedBy = "dcr"
	ManagedByCimd  ManagedBy = "cimd"
)
...
type OAuth2ClientCreatedResponse struct {
	// ClientID carries the server's client_id field.
	ClientID string `json:"client_id"`
	// ClientSecret The plaintext client secret, shown exactly once. T21.2 — **absent**
	// for a client registered with `token_endpoint_auth_method: none`. A
	// public client is created with no secret, so there is nothing to show;
	// the member is omitted rather than sent as `""`, which an operator (or
	// an SDK) would reasonably read as a secret that happens to be empty.
	// Every confidential registration — that is, every registration that
	// existed before T21.2 — carries it exactly as before.
	//
	// Secret. Redacted from every fmt verb, log line and JSON rendering; the
	// raw value never leaves this package except on the wire.
...
type OAuth2ClientResponse struct {
	// AllowedResources T21.3 — echoed in its stored, normalised form, so an operator
	// auditing which audiences a client may mint tokens for reads the strings
	// the server actually compares rather than the ones they typed.
	...
	JWKSURI *string `json:"jwks_uri,omitempty"`
	// LastAuthorizedAt T21.4 — when this client was last issued an authorization code, for
	// the sweeper that deletes self-registered clients nobody uses. Always
	// absent for an `admin` client: the stamp is written only for a
	...
	// ManagedBy T21.4 / D5 — who created this registration: `admin`, `dcr` or `cimd`.
	...
	// kind the unused-client sweeper touches. Read-only. There is no
	// corresponding member on the update DTO: a registration's provenance is
	// a fact about how it came to exist, and a field that could be edited to
	// `admin` would be a field that launders one.
...
// OIDCPolicy OpenID Connect surface controls (X7 G8, plan §4.6/§4.8; T21.4).
...
// more permissive: * [`Self::sensitive_scopes_enabled`], validated
...
// [`dcr_ttl_strictness`]. **Not ordered**, therefore never validated
// against the baseline and never clamped: * [`Self::default_locale`]. A
// language is a presentation preference; there is no sense in which
// Italian is stricter than French. * [`Self::dcr_allowed_scopes`],
// [`Self::dcr_allowed_redirect_hosts`] and
// [`Self::external_client_allowed_resources`]. Each names per-tenant
// resources — *this* tenant's MCP servers, *this* tenant's callback
// hosts — and there is no sense in which one such list is stricter than
// another. A subset rule would force an organization to enumerate every
// tenant's resource servers in its own baseline before any tenant could
// name one. The model's rule is "a tenant may only be more restrictive",
// which binds every field that *has* a restrictiveness; a field that has
// none cannot violate it. One cross-field interlock spans both groups and
...
type OIDCPolicy struct {
	...
	// DcrAllowedScopes T21.4 — the scopes a self-registered client may ask for. A `scope` a
	...
	// has turned registration on without deciding what it grants. May not
	...
	// DynamicRegistration T21.4 — whether a client may register itself (RFC 7591), and on what
	// terms. `disabled` unless somebody says otherwise (I1).
	DynamicRegistration *string `json:"dynamic_registration,omitempty"`
	// ExternalClientAllowedResources **D3** — the audiences an externally registered client may address.
	...
	// than one the registration request makes. Empty means an externally
	// registered client can obtain only today's `axiam:user` tokens — which
	// AXIAM's own APIs accept. That is why the interlock exists: the empty
	// list is not a safe default for an *open* registration endpoint, it is
...
type SetOrgSettings struct {
	...
	// DynamicRegistration carries the server's dynamic_registration field.
	DynamicRegistration *string `json:"dynamic_registration,omitempty"`
	// EmailVerificationGracePeriodHours carries the server's email_verification_grace_period_hours field.
	EmailVerificationGracePeriodHours int `json:"email_verification_grace_period_hours"`
	// EmailVerificationRequired carries the server's email_verification_required field.
